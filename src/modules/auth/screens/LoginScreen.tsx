    ...
    loginMutation.mutate(
      { username:username?.toLowerCase(), password },
      {
        onSuccess: () => {
          ...
          // AuthContext handles routing to (protected)
        },
        onError: (error: any) => {
    ...
    <ThemedView className="flex-1 justify-center px-8 bg-slate-50 dark:bg-slate-900">
      ...
      <View className="space-y-4">
        ...
        <View className="flex-row justify-center mt-6">
          <ThemedText className="text-slate-500 dark:text-slate-400">Don't have an account? </ThemedText>
          <TouchableOpacity onPress={() => router.push('/(auth)/register')}>
            <ThemedText className="text-blue-600 font-bold">Sign Up</ThemedText>
          </TouchableOpacity>
        </View>
      </View>
    </ThemedView>
